...
class CharacterSetMappingsTest
{
    @Test
    void testDefaultJavaToHttp()
    {
        assertEquals("iso-8859-1", CharacterSetMappings.mapCharsetToHttp(null), "default encoding");
    }

    @Test
    void testDefaultHttpToJava()
    {
        assertEquals(System.getProperty("file.encoding", "ISO8859_1"), CharacterSetMappings.mapHttpToCharset(null), "default encoding");
    }

    @Test
    void testJavaToHttp()
    {
        Map<String,String> javaToHttpTable = new HashMap<>();
        ...
        for(Entry<String, String> entry : javaToHttpTable.entrySet())
        {
            assertEquals(entry.getValue(), CharacterSetMappings.mapCharsetToHttp(entry.getKey()), entry.getKey());
        }
    }
    ...
    void testHttp2Java()
    {
        ...
        Map<String, String> httpToJavaTable = new HashMap<>();
        ...
        for(Entry<String, String> entry : httpToJavaTable.entrySet())
        {
            assertEquals(entry.getValue(), CharacterSetMappings.mapHttpToCharset(entry.getKey()), entry.getKey());
        }
    }
}
